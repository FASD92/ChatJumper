...
- 첫 실행은 최신 사용자 질문으로 이동한다. 이후 반복 실행은 DOM 질문 목록을 매번 다시 수집하되, 직전 선택 인덱스를 기준으로 바로 이전 사용자 질문을 순회한다. 사용자가 wheel, touch, scroll key로 직접 위치를 바꾸면 순회 상태를 reset하고 다음 실행은 현재 viewport 기준으로 다시 시작한다.

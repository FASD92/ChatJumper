...
- 수동 스크롤 후 실행은 현재 viewport 기준으로 계산한다. DOM 질문 목록을 매번 다시 수집하고, viewport 상단 기준선보다 완전히 위로 지나간 사용자 질문 중 가장 최신 질문으로 이동한다. 따라서 현재 스크롤이 최신 질문과 그 이전 질문 사이에 있으면 그 이전 질문으로 이동한다.
- J 버튼 또는 단축키를 연속 실행할 때는 직전 수집한 질문 목록 캐시를 사용해 최신 질문에서 바로 이전 질문으로 순서대로 이동한다. 사용자가 wheel, touch, scroll key로 직접 위치를 바꾸면 이 연속 실행 상태를 reset한다.

### Keyboard Shortcut

단축키는 빠른 점프 실행 경로로 유지한다. 기본값은 manifest의 command 설정을 따른다.
Floating Button과 같은 이동 규칙을 사용하며, 수동 스크롤 후에는 현재 viewport 기준으로, 연속 실행 중에는 캐시된 질문 순서 기준으로 이전 사용자 질문을 찾는다.
